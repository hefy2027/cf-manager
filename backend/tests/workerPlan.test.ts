...
import { isPaidPlan, normalizeWorkerPlan } from '../src/models/account';
import { subdomainPrefix } from '../src/services/workerSubdomain';
import { cachePaidModelNames, isPaidModelName, modelRequiresWorkersPaid } from '../src/services/aiService';
import { mapWorkerPlanFromSubscriptions } from '../src/services/accountProbe';
...
describe('worker_plan：计划类型判定', () => {
  ...
  it('非法值一律规范化为 free', () => {
    ...
    expect(normalizeWorkerPlan(123)).toBe('free');
  });
});
...
describe('付费模型名缓存（账号路由据此过滤账号）', () => {
  ...
  it('按 CF 元数据的 require_workers_paid 标记（含字符串 "true"）建立缓存', () => {
    cachePaidModelNames([
      { id: 'paid-model', properties: [{ property_id: 'require_workers_paid', value: 'true' }] },
      { name: 'paid-model-2', require_workers_paid: true },
      { id: 'free-model', properties: [{ property_id: 'require_workers_paid', value: 'false' }] },
    ]);
