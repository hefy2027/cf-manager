 ...
 *
 * 与 worker/src/services/workerSubdomain.ts 保持同构。
 */
...
export async function getAccountSubdomain(account: Account): Promise<string> {
  ...
  try {
    const resp = await proxyFetch(
      `${CF_BASE}/accounts/${account.account_id}/workers/subdomain`,
      { headers: { 'Content-Type': 'application/json', ...getAuthHeaders(account) } },
      REQUEST_TIMEOUT_MS, undefined, account,
    );
    if (!resp.ok) return '';
 ...
 */
export async function ensureAccountSubdomain(account: Account): Promise<{ subdomain: string; error?: string }> {
  ...
  for (const prefix of candidatePrefixes(base)) {
    try {
      ...
      if (resp.ok) {
        ...
        appLogger.info(`[WorkerSubdomain] Registered workers.dev subdomain "${subdomain}" for account "${account.name}"`);
        return { subdomain };
      }
      ...
      if (!looksLikeUnavailable(resp.status, body)) {
        appLogger.warn(`[WorkerSubdomain] Failed to register subdomain="${prefix}" for "${account.name}": ${lastError}`);
        return { subdomain: '', error: lastError };
      }
      appLogger.info(`[WorkerSubdomain] "subdomain=${prefix}" unavailable for "${account.name}", trying next candidate`);
    } catch (err: any) {
      lastError = err?.message || String(err);
      appLogger.warn(`[WorkerSubdomain] subdomain registration error for "${account.name}": ${lastError}`);
      return { subdomain: '', error: lastError };
    }
  }

  appLogger.warn(`[WorkerSubdomain] All subdomain candidates are taken for "${account.name}": ${lastError}`);
